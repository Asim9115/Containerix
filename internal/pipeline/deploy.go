...
func Deploy(jobId string, url string) (string, error){
	...
	if sm, ok := state.SB.Sandbox.(*sandbox.SandboxManager); ok {
		if err := cgroup.AddProcess(sm.Name, pid); err != nil {
			return "", err
		}
	}
	...
	//12. store container in sandbox state via type assertion
	if sm, ok := state.SB.Sandbox.(*sandbox.SandboxManager); ok {
		sm.Containers[cfg.Name] = &types.Container{
			ID:     cfg.Name,
			Config: cfg,
			Status: "running",
		}
